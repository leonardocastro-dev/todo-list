...
onMounted(async () => {
  ...
  taskStore.setCurrentProject(projectId.value, user.value?.uid)
})

watch(
  () => projectId.value,
  async (newProjectId) => {
    taskStore.setCurrentProject(newProjectId, user.value?.uid)
  }
)
</script>
  ...
  <div class="max-w-6xl mx-auto">
    <header class="mb-8">
      ...
      <div v-else class="text-center py-8">
        ...
        <Button @click="router.push('/dashboard/projects')" class="mt-4">
          Go to Projects
        </Button>
      </div>
    </header>
